...
The functions `lpnet.input.tree` allows user to use an arbitrary phylogenetic tree to repalce the tree which constructed in `lpnet`. In addition the input tree's class should be `phylo` and the `edge` block is necessary.
