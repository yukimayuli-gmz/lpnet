...
C   
  ...
  430 continue
      ...
      END  
C     ------------------------------------------------------------------
C     try to do nnls by only input b
C     ------------------------------------------------------------------
      SUBROUTINE nnls_b (Taxanum,MLine,B,X,RNORM,W,ZZ,INDEX,MODE,NSETP)
C     ------------------------------------------------------------------
	  integer Taxanum, MLine, Mi, Mj, Bi, Bj, Bk, Bl, Mk, Ml
	  double precision Tnum
c	  double precision CorrespondM(MLine,MLine)
	  double precision CorrespondB(Taxanum,Taxanum)
      integer M, MDA, MODE,N, NSETP
      integer INDEX(MLine)  
      double precision A(MLine,MLine), B(MLine)
	  double precision W(MLine), X(MLine), ZZ(MLine)  
c     integer INDEX(*)  
	  ...
	  DO Mi=1,Taxanum-1
	     DO Mj=Mi+1,Taxanum
		    IF (Mi .eq. 1) then
               DO Bi=1,Mj-1
			      DO Bj=1,Bi
				     CorrespondB(Bi,Bj)=0.0d0
				  END DO
			   END DO
			   DO Bi=Mj,Taxanum
			      DO Bj=1,Mj-1
				     CorrespondB(Bi,Bj)=1.0d0
				  END DO
				  DO Bj=Mj,Bi
				     CorrespondB(Bi,Bj)=0.0d0
				  END DO
			...
			DO Bk=2,Taxanum
			   DO Bl=1,Bk-1
 			      Tnum=CorrespondB(Bk,Bl)
				  Mk=(Bk-1)*(Bk-2)/2+Bl
				  Ml=(Taxanum*2-Mi)*(Mi-1)/2+Mj-Mi
				  A(Mk,Ml)=Tnum
c                 CorrespondM((Bk-1)*(Bk-2)/2+Bl,(Taxanum*2-Mi)*(Mi-1)/2+Mj-Mi)=CorrespondB(Bk,Bl)
			   END DO
			END DO
		 END DO
	  END DO
	  CALL NNLS (A,MDA,M,N,B,X,RNORM,W,ZZ,INDEX,MODE,NSETP)
	  RETURN
	  END